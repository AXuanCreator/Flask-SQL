...
推荐使用 **Miniconda** 作为Python包管理器

在Miniconda下载网页 [Miniconda — Anaconda documentation](https://docs.anaconda.com/miniconda/) 中找到自己系统的安装包，并根据安装提示安装

### 创建虚拟环境并下载必要的包

在 **CMD** (快捷键:Windows+R输入CMD) 中键入

```cmd
conda create -n [虚拟环境名称] python=[PYTHON的版本]
# 示例：conda create -n flask-web python=3.11 -y

conda activate [虚拟环境名称]
# 示例: conda activate flask-web
...
在 Pycharm 新建项目后，可以在 **主页面右下角** 或 **设置中** 找到解释器设置并添加Conda虚拟环境
...
运行程序后，在浏览器中输入 **127.0.0.1:5000** 即可打开一个写有 Hello World 的网页
...
* **PORT**：SQL所需要的端口，此端口在创建SQL时指定，对于MySQL而言，默认端口号为`3306`。同时，也可以通过命令行的方式查询。
    ...
    6对于MySQL，可按下Windows键后在搜索框中搜索MySQL，打开命令行工具
...
ORM（对象关系映射）是一种编程技术，它将关系型数据库中的表格和数据映射到编程语言中的对象模型上。简单来说，ORM模型就是 **数据表**
...
![image-20240518141831343](assets/image-20240518141831343.png)
...
在前几个例子中，我们使用了一下语句来将ORM模型同步到数据库中
